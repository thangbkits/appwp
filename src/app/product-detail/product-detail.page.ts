...
export class ProductDetailPage implements OnInit {
    ...
    ngOnInit() {
        this.config.getOptions().then((acf: any) => {
            let afda = acf
            this.acfget.push(afda);
            // this.acfget = acf;
            this.ttKM = acf.acf.tin_khuyen_mai_chung;
            this.fixacf = acf.acf;
            console.log(this.ttKM, 'Acf options');
            console.log(this.acfget.push(afda), 'Acf =====');
        }, err => {
            console.log(err);
        });
        // this.acfget = JSON.parse(JSON.stringify(this.getProductData(this.pId)));
    ...
    addToCartProduct() {
        ...
        if (this.product.type == 'grouped') {
            ...
            if (total == 0) this.shared.translateString("Please Add Quantity").then((res) => {
    ...
    goContact() {
        this.router.navigateByUrl('tabs/contact');
    }
}
